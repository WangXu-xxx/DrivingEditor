...
class GaussianModel:
    ...
    def __init__(self, args):
        ...
        self.dynamic_random_init_point = args.dynamic_random_init_point
    ...
    def get_xyz_SHM(self, t, times=1):
        a = 1 / self.T * np.pi * 2
        return self._xyz + (self._velocity * torch.sin((t - self._t) * a) / a) * times
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        r = 1 / r_1
        pts_sph = torch.stack(
            [r * torch.cos(theta) * torch.cos(phi), r * torch.sin(theta) * torch.cos(phi), r * torch.sin(phi)],
            dim=-1).cuda()
        ...
        if pcd.time is None or pcd.time.shape[0] != fused_point_cloud.shape[0]:
            ...
            if self.t_init < 1:
                random_times = (torch.rand(fused_point_cloud.shape[0] - pcd.points.shape[0], 1,
                                           device="cuda") * 1.2 - 0.1) * (
                                       self.time_duration[1] - self.time_duration[0]) + self.time_duration[0]
    ...
    def add_setup(self, mask3d):

        # mask3d = mask3d.bool().squeeze()

        # Extracting subsets using the mask
        xyz_sub = self._xyz.detach().clone()
        xyz_sub[:, 1] += 1
        features_dc_sub = self._features_dc.detach().clone()
        features_rest_sub = self._features_rest.detach().clone()
        opacity_sub = self._opacity.detach().clone()
        scaling_sub = self._scaling.detach().clone()
        rotation_sub = self._rotation.detach().clone()
        t_sub = self._t.detach().clone()
        scaling_t_sub = self._scaling_t.detach().clone()
        velocity_sub = self._velocity.detach().clone()
        radii_sub = self.max_radii2D.detach().clone()
        t_g_sub = self.t_gradient_accum.detach().clone()
        xyz_g_sub = self.xyz_gradient_accum.detach().clone()
        xyz_sub = torch.cat((self._xyz.detach(), xyz_sub), dim=0)
        features_dc_sub = torch.cat((self._features_dc.detach(), features_dc_sub), dim=0)
        features_rest_sub = torch.cat((self._features_rest.detach(), features_rest_sub), dim=0)
        opacity_sub = torch.cat((self._opacity.detach(), opacity_sub), dim=0)
        scaling_sub = torch.cat((self._scaling.detach(), scaling_sub), dim=0)
        rotation_sub = torch.cat((self._rotation.detach(), rotation_sub), dim=0)
        t_sub = torch.cat((self._t.detach(), t_sub), dim=0)
        scaling_t_sub = torch.cat((self._scaling_t.detach(), scaling_t_sub), dim=0)
        velocity_sub = torch.cat((self._velocity.detach(), velocity_sub), dim=0)
        radii_sub = torch.cat((self.max_radii2D.detach(), radii_sub), dim=0)
        t_g_sub = torch.cat((self.t_gradient_accum.detach(), t_g_sub), dim=0)
        xyz_g_sub = torch.cat((self.xyz_gradient_accum.detach(), xyz_g_sub), dim=0)
        ...
        self._t = nn.Parameter(set_requires_grad(t_sub, False))
        self._scaling_t = nn.Parameter(set_requires_grad(scaling_t_sub, False))
        self._velocity = nn.Parameter(set_requires_grad(velocity_sub, False))
        self.max_radii2D = nn.Parameter(set_requires_grad(radii_sub, False))
        self.t_gradient_accum = nn.Parameter(set_requires_grad(t_g_sub, False))
        self.xyz_gradient_accum = nn.Parameter(set_requires_grad(xyz_g_sub, False))

    def removal_setup(self, mask3d, training_args):
        ...
        t_sub = self._t[mask3d].detach()
        scaling_t_sub = self._scaling_t[mask3d].detach()
        velocity_sub = self._velocity[mask3d].detach()
        def set_requires_grad(tensor, requires_grad):
            """Returns a new tensor with the specified requires_grad setting."""
            return tensor.detach().clone().requires_grad_(requires_grad)
    ...
    def save_ply(self, path, time, time_shift):
        self.mkdir_p(os.path.dirname(path))
        if time_shift is not None:
            xyz = (self.get_xyz_SHM(time - time_shift) + self.get_inst_velocity * time_shift).detach().cpu().numpy()
            marginal_t = self.get_marginal_t(time - time_shift)
        else:
        ...
        opacities = (self._opacity * marginal_t).detach().cpu().numpy()
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size, max_grad_t=None, prune_only=False):
        ...
        if max_screen_size:
            ...
            big_points_ws = self.get_scaling.max(
                dim=1).values > self.big_point_threshold * extent * scale_factor  ## ori 0.1
    ...
    def update_rotation(self, rotation):
        self._rotation = torch.tensor(rotation, device='cuda', dtype=torch.float32)


class Dynamic_GaussianModel(GaussianModel):
    def __init__(self, sh_degree: int):
        super(Dynamic_GaussianModel, self).__init__(sh_degree)
    ...
    def initialize(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        r_max = 100000
        r_min = 2
        num_sph = self.dynamic_random_init_point
        ...
        r_rec = r_min
        num_rec = self.dynamic_random_init_point
        pts_rec = torch.stack([r_rec * (torch.rand(num_rec) - 0.5), r_rec * (torch.rand(num_rec) - 0.5),
                               r_rec * (torch.rand(num_rec))], dim=-1).cuda()
    ...
    def training_setup(self, training_args):
        ...
        l = [
            {'params': [self._xyz], 'lr': training_args.position_lr_init * 10, "name": "xyz"},
            {'params': [self._features_dc], 'lr': training_args.feature_lr, "name": "f_dc"},
            {'params': [self._features_rest], 'lr': training_args.feature_lr / 20.0, "name": "f_rest"},
            {'params': [self._opacity], 'lr': training_args.opacity_lr, "name": "opacity"},
            {'params': [self._scaling], 'lr': training_args.scaling_lr, "name": "scaling"},
        ...
        self.optimizer = torch.optim.Adam(l, lr=0.0, eps=1e-15)
        self.xyz_scheduler_args = get_expon_lr_func(lr_init=training_args.position_lr_init * 10,
